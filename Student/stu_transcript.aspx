<%@ Page Language="C#" AutoEventWireup="true" CodeFile="stu_transcript.aspx.cs" Inherits="stu_transcript" %>

 <!DOCTYPE html>
<html>
<head>

	<title>Student|HOME</title>
	<script src="https://cdn.jsdelivr.net/npm/@popperjs/core@2.11.7/dist/umd/popper.min.js" integrity="sha384-zYPOMqeu1DAVkHiLqWBUTcbYfZ8osu1Nd6Z89ify25QV9guujx43ITvfi12/QExE" crossorigin="anonymous"></script>
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha3/dist/js/bootstrap.min.js" integrity="sha384-Y4oOpwW3duJdCWv5ly8SCFYWqFDsfob/3GkgExXKV4idmbt98QcxXYs9UoXAB7BZ" crossorigin="anonymous"></script>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-KK94CHFLLe+nY2dmCWGMq91rCGa5gtU4mk92HdvYe+M/SXH301p5ILy+dN9+nJOZ" crossorigin="anonymous">	
<link rel="stylesheet" type="text/css" href="homestyle.css">
</head>
<body>
	<img src="./4.jpg" alt="error" class="theme">
	<nav>
		<ul>
			<li><a href="./home.html">Home</a></li>
			<li><a href="#">Attendance</a></li>
			<li><a href="./marks.html">Marks</a></li>
			<li><a href="./transcript.html">Transcript</a></li>
            <li><a href="#">CGPA Comparison</a></li>
		
		</ul>
    </nav>

    <title>Student Transcript</title>
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<link rel="stylesheet" type="text/css" href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0/dist/css/bootstrap.min.css">

	<div class="container mt-5">
		<h2 class="text-center">Transcript</h2>
		<ul class="nav nav-tabs">
			<li class="nav-item">
				<a class="nav-link active" href="#semester1" data-bs-toggle="tab">Semester 1</a>
			</li>
			<!-- <li class="nav-item">
				<a class="nav-link" href="#semester2" data-bs-toggle="tab">Semester 2</a>
			</li>
			<li class="nav-item">
				<a class="nav-link" href="#semester3" data-bs-toggle="tab">Semester 3</a>
			</li> -->
		</ul>
		<div class="tab-content mt-3">
			<div class="tab-pane fade show active" id="semester1">
				<table class="table table-bordered">
					<thead>
						<tr class="table-head">
							<th>Course Code</th>
							<th>Course Title</th>
							<th>Credit Hours</th>
							<th>Grade</th>
						</tr>
					</thead>
					<tbody class="container-std-detail">
						<tr>
							<td>CS101</td>
							<td>Introduction to Computer Science</td>
							<td>3</td>
							<td>A</td>
						</tr>
						<tr>
							<td>MA201</td>
							<td>Calculus I</td>
							<td>4</td>
							<td>B+</td>
						</tr>
						<tr>
							<td>ENG101</td>
							<td>English Composition I</td>
							<td>3</td>
							<td>A-</td>
						</tr>
						<tr>
							<td>PHY101</td>
							<td>Physics I</td>
							<td>4</td>
							<td>B</td>
						</tr>
						<tr>
							<td>PSY101</td>
							<td>Introduction to Psychology</td>
							<td>3</td>
							<td>C</td>
						</tr>
						<tr>
							<td>Total</td>
							<td></td>
							<td>17</td>
							<td>3.34</td>
						</tr>
					</tbody>
				</table>
			</div>
			<!-- <div class="tab-pane fade" id="semester2">
				<table class="table table-bordered">
					<thead>
						<tr>
							<th>Course Code</th>
							<th>Course Title</th>
							<th>Credit Hours</th>
							<th>Grade</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td>CS201</td>
							<td>Data Structures and Algorithms</td>
							<td>3</td>
							<td>B+</td>
                        </tr>
						<tr>
							<td>CS202</td>
							<td>Discrete Mathematics</td>
							<td>3</td>
							<td>A</td>
						</tr>
						<tr>
							<td>ENG201</td>
							<td>English Composition II</td>
							<td>3</td>
							<td>A-</td>
						</tr>
						<tr>
							<td>PHY201</td>
							<td>Physics II</td>
							<td>4</td>
							<td>C+</td>
						</tr>
						<tr>
							<td>CS205</td>
							<td>Web Development</td>
							<td>3</td>
							<td>B</td>
						</tr>
						<tr>
							<td>Total</td>
							<td></td>
							<td>16</td>
							<td>2.87</td>
						</tr>
					</tbody>
				</table>
			</div>
			<div class="tab-pane fade" id="semester3">
				<table class="table table-bordered">
					<thead>
						<tr>
							<th>Course Code</th>
							<th>Course Title</th>
							<th>Credit Hours</th>
							<th>Grade</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td>CS301</td>
							<td>Database Systems</td>
							<td>3</td>
							<td>A-</td>
						</tr>
						<tr>
							<td>CS303</td>
							<td>Operating Systems</td>
							<td>3</td>
							<td>B+</td>
						</tr>
						<tr>
							<td>CS311</td>
							<td>Software Engineering</td>
							<td>3</td>
							<td>A</td>
						</tr>
						<tr>
							<td>CS341</td>
							<td>Artificial Intelligence</td>
							<td>3</td>
							<td>C+</td>
						</tr>
						<tr>
							<td>STA301</td>
							<td>Statistics and Probability</td>
							<td>3</td>
							<td>B-</td>
						</tr>
						<tr>
							<td>Total</td>
							<td></td>
							<td>15</td>
							<td>3.07</td>
						</tr>
					</tbody>
				</table>
			</div> -->
		</div>
	</div>
	<script type="text/javascript" src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>

